...
async function upsertCustomer(env: Env, c: Partial<CustomerInfo> & { telegramUserId: number }) {
  ...
  else all.push({ phone: '', streetNumber: '', streetName: '', address: '', unit: '', passcode: '', credits: 0, surcharge: 0, name: c.name ?? '', username: c.username ?? '', telegramUserId: c.telegramUserId })
  await env.MHC_KV.put('customers', JSON.stringify(all))
}
